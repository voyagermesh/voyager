...
These are the supported Certificate Issuer:
...
Here we will show issuing certificates from Let's Encrypt using ACME protocol. For others, click on the link of the respective issuers.

#### [acme](https://docs.cert-manager.io/en/latest/tasks/issuers/setup-acme/index.html)

The ACME Issuer type represents a single Account registered with the ACME server. When you create a new ACME Issuer, cert-manager will generate a private key which is used to identify you with the ACME server.
...
Here, put your email address here. This `email` will be used to register for your let's encrypt account and `privateKeySecretRef` will contain this accounts private key.
...
For more to know, click on [this link](https://letsencrypt.org/docs/rate-limits/)
