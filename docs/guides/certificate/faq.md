...
Voyager supports issuing wildcard certificates using Let's Encrypt since version 7.0.0-rc.3. To issue wildcard domain, set the domain name in your certificate crd as `"*.yourdomain.com"`. Please note that wildcard domain is only supported with DNS challenges and can't be issued via HTTP challenge.
