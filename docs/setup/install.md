...
-p, --provider=PROVIDER            specify a cloud provider
    ...
    --haproxy-image-tag            tag of Docker image containing HAProxy binary (default: 1.8.12-7.2.0-alpine)
...
By default, Voyager uses Alpine based HAProxy image (1.8.12-7.2.0-alpine). But you can also Debian based image for HAProxy by setting --haproxy-image-tag=1.8.12-7.2.0 flag.
