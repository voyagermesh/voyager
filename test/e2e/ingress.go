...
func (s *IngressTestSuit) TestIngressEnsureTPR() error {
	var err error
	for it := 0; it < 10; it++ {
		log.Infoln(it, "Trying to get ingress.networking.appscode.com")
		tpr, err := s.t.KubeClient.ExtensionsV1beta1().ThirdPartyResources().Get("ingress.networking.appscode.com", metav1.GetOptions{})
		if err == nil {
