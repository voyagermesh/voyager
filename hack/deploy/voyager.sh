...
if [ -z "$VOYAGER_TEMPLATE_CONFIGMAP" ]; then
    curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-rc.9/hack/deploy/operator.yaml | envsubst | kubectl apply -f -
else
    ...
    curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-rc.9/hack/deploy/operator-with-custom-tpl.yaml | envsubst | kubectl apply -f -
fi

if [ "$VOYAGER_ENABLE_RBAC" = true ]; then
    curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-rc.9/hack/deploy/rbac.yaml | envsubst | kubectl apply -f -
fi

if [ "$VOYAGER_RUN_ON_MASTER" -eq 1 ]; then
    kubectl patch deploy voyager-operator -n $VOYAGER_NAMESPACE \
      --patch="$(curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-rc.9/hack/deploy/run-on-master.yaml)"
fi
