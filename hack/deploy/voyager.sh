...
if [ "$VOYAGER_ENABLE_ADMISSION_WEBHOOK" = true ]; then
    ...
    curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-alpha.0/hack/deploy/admission/operator.yaml | envsubst | kubectl apply -f -
else
    curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-alpha.0/hack/deploy/operator.yaml | envsubst | kubectl apply -f -
fi
...
if [ -n "$VOYAGER_TEMPLATE_CONFIGMAP" ]; then
    ...
    kubectl patch deploy voyager-operator -n $VOYAGER_NAMESPACE \
      --patch="$(curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-alpha.0/hack/deploy/use-custom-tpl.yaml | envsubst)"
fi
...
if [ "$VOYAGER_ENABLE_RBAC" = true ]; then
    ...
    curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-alpha.0/hack/deploy/rbac-list.yaml | envsubst | kubectl auth reconcile -f -

    if [ "$VOYAGER_ENABLE_ADMISSION_WEBHOOK" = true ]; then
        curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-alpha.0/hack/deploy/admission/rbac-list.yaml | envsubst | kubectl auth reconcile -f -
    fi
fi

if [ "$VOYAGER_RUN_ON_MASTER" -eq 1 ]; then
    kubectl patch deploy voyager-operator -n $VOYAGER_NAMESPACE \
      --patch="$(curl -fsSL https://raw.githubusercontent.com/appscode/voyager/5.0.0-alpha.0/hack/deploy/run-on-master.yaml)"
fi
