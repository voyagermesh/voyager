...
type IngressLister interface {
	...
	List(selector labels.Selector) (ret []*voyager.Ingress, err error)
...
// List lists all Ingresses in the indexer.
func (s *ingressLister) List(selector labels.Selector) (ret []*voyager.Ingress, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*voyager.Ingress))
	})
	return ret, err
}
...
type IngressNamespaceLister interface {
	...
	Get(name string) (*voyager.Ingress, error)
...
// List lists all Ingresses in the indexer for a given namespace.
func (s ingressNamespaceLister) List(selector labels.Selector) (ret []*voyager.Ingress, err error) {
...
// Get retrieves the Ingress from the indexer for a given namespace and name.
func (s ingressNamespaceLister) Get(name string) (*voyager.Ingress, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(voyager.Resource("ingress"), name)
	}
	return obj.(*voyager.Ingress), nil
}
