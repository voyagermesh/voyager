...
func (c *loadBalancerController) Reconcile() error {
	...
	if _, vt, err := c.ensurePods(); err != nil {
		c.recorder.Eventf(
			...
			"Failed to ensure LoadBalancer Pods, Reason: %s",
			err.Error(),
		)
		return errors.FromErr(err).Err()
	} else if vt != kutil.VerbUnchanged {
		c.recorder.Eventf(
			...
			"Successfully %s LoadBalancer Pods",
			vt,
		)
	}
	...
	if _, vt, err := c.ensureService(); err != nil {
		c.recorder.Eventf(
			...
			"Failed to ensure LoadBalancer Service, Reason: %s",
			err.Error(),
		)
		return errors.FromErr(err).Err()
	} else if vt != kutil.VerbUnchanged {
		c.recorder.Eventf(
			...
			"Successfully %s LoadBalancer Service",
			vt,
		)
	}
