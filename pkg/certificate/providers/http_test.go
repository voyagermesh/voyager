...
func TestNotFound(t *testing.T) {
	...
	resp, err := http.Get(fmt.Sprintf("http://127.1.1.1:%d/.well-known/acme-challenge/token", ACMEResponderPort))
	if err != nil {
...
func TestFound(t *testing.T) {
	defaultHTTPProvider.serve()
	time.Sleep(time.Second * 5)
	defaultHTTPProvider.Present(fmt.Sprintf("127.1.1.1:%d", ACMEResponderPort), "token", "key")
	resp, err := http.Get(fmt.Sprintf("http://127.1.1.1:%d/.well-known/acme-challenge/token", ACMEResponderPort))
	if err != nil {
		t.Fatal("expected Nil, found", err)
	}
